...
class QQBot:
    ...
    def fetchGroups(self):
        ...
        self.groupStr = '群列表:\n' + str(self.groups)
        QLogger('获取群列表成功，共 %d 个朋友' % len(self.groups))
    ...
    def fetchDiscusses(self):
        ...
        QLogger('获取讨论组列表成功，共 %d 个讨论组' % len(self.discusses))
    ...
    def _send(self, msgType, to_uin, msg):
        self.msgId += 1        
        if self.msgId % 10 == 0:
            QLogger('已连续发送10条消息，强制 sleep 10秒，请等待...')
            time.sleep(10)
        else:
        ...
        try:
            ...
            name ='未知qq'
            return None
        print('向%s(%s) 回复消息成功' % (name[1], name[0]))
